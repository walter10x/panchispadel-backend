import { JoinMatchUseCase } from './join-match.use-case';
...
describe('JoinMatchUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new JoinMatchUseCase(repository, notifService, wsGateway);
  });

  it('agrega jugador al match como pendiente, guarda y notifica', async () => {
  ...
  it('lanza NotFoundError si el match no existe', async () => {
    ...
    await expect(
      useCase.execute({ matchId: 'no-existe' }, 'user-2', 'user2@test.com', 'Player Name'),
    ).rejects.toThrow(NotFoundError);
    expect(repository.save).not.toHaveBeenCalled();
    expect(notifService.notifyPlayerJoined).not.toHaveBeenCalled();
  });

  it('no notifica match full cuando jugador se une como pendiente', async () => {
    const match = Match.create({
      creatorId: 'user-1',
      creatorEmail: 'user-1@test.com',
      clubId: 'club-1',
    ...
    expect(notifService.notifyMatchFull).not.toHaveBeenCalled();
  });
});
